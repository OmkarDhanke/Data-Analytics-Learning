...
SELECT
    ...
    car_model,
    sale_price,
    sum(sale_price) OVER() as TOTAL_revenue
FROM
	velocitymotors_db.car_sales;
...
SELECT
	...
	rep_id,
    sale_price,
    SUM(sale_price) OVER(PARTITION BY rep_id) as "total revenue generated"
FROM
	velocitymotors_db.car_sales;
    
-- Question 3: Running Total (Cumulative Sum)
-- List all sales in chronological order (sale_date). 
-- Calculate a running total of Cumulative for the dealership over time.
-- Vague Hint: A window sum combined with a chronological sort inside the window creates a running total.
SELECT
	sale_id,
    sale_date,
    SUM(sale_price) OVER(ORDER BY sale_date) as Cumulative_Cumulative
FROM
	velocitymotors_db.car_sales;
...
SELECT
	sale_id,
    sale_date,
    ROW_NUMBER() OVER(ORDER BY sale_date) as Chronological
FROM
	velocitymotors_db.car_sales;
...
SELECT
	...
	rep_id,
    sale_price,
    ROW_NUMBER() OVER(PARTITION BY rep_id ORDER BY sale_date) as "rep's personal milestones"
FROM
	velocitymotors_db.car_sales;
...
SELECT
	c.rep_id,
    ts.rep_revenue,
    RANK() OVER(ORDER BY ts.rep_revenue ) as rep_rank
FROM
	velocitymotors_db.car_sales as c
LEFT JOIN total_revenue ts
ON ts.rep_id = c.rep_id;
...
WITH ranked_sales AS (
    SELECT
        ...
        RANK() OVER (
            ...
            ORDER BY c.sale_price DESC
        ) AS rnk
    FROM velocitymotors_db.car_sales AS c
    LEFT JOIN velocitymotors_db.sales_reps AS s
        ON c.rep_id = s.rep_id
)
SELECT
    branch,
    car_model,
...
SELECT
    ...
    sale_price,
    ROUND(
        (sale_price / SUM(sale_price) OVER()) * 100, 2
...
SELECT
    ...
    car_model,
    sale_price,
    NTILE(3) OVER(ORDER BY sale_price DESC) AS price_tier
FROM
    velocitymotors_db.car_sales;