...
Eorder as (
	SELECT userid, Total_Order FROM ordercount
    WHERE Total_Order > 1
)
SELECT 
(SUM(CASE WHEN Total_Order > 1 THEN 1 ELSE 0 END) / COUNT(*)) * 100 AS repeat_purchaser_percentage 
FROM ordercount;
...
SELECT
    ...
    u.signup_date,
    o.order_date,
    datediff(o.order_date,u.signup_date) as days
FROM users as u
INNER JOIN orders as o
on u.user_id = o.user_id
WHERE u.signup_date = o.order_date;

SELECT
    u.user_id,
    u.signup_date,
    MIN(o.order_date) AS first_order_date, -- Grabs the absolute earliest order date
    DATEDIFF(MIN(o.order_date), u.signup_date) AS days_to_first_purchase
FROM users AS u
INNER JOIN orders AS o
    ON u.user_id = o.user_id
GROUP BY 
...
SELECT
	SUM(CASE WHen event_type = 'view_item' THEN 1 ELSE 0 END) as Total_Views,
